...
std::map<std::vector<int>, std::map<bool, std::string>> regex_findrmid(std::string searched, std::string x) {
    ...
    if (searched[cnt] == '{') {
      ...
      if (searched[cnt] == '?') {
        ...
        if (rslt_mp->first) {
          idx_v = cur_it->first;
          rtn_lst_cnt = idx_v[1];
          for (cnt2 = 0; cnt2 < n - rtn_lst_cnt - 1; ++cnt2) {
            cur_x.pop_back();
          };
        };
      };
  ...
  std::map<std::vector<std::vector<unsigned int>>, std::vector<std::string>> bgn_mp = regex_findr_bgn(searched, cur_x);
  std::map<std::vector<std::vector<unsigned int>>, std::vector<std::string>>::iterator bgn_it = bgn_mp.begin();
  std::vector<std::string> hmn_str_idxv = bgn_it->second;
  std::vector<std::vector<unsigned int>> hmn_idxv = bgn_it->first;
  rtn_str = hmn_str_idxv[0];
